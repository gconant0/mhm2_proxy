...
upcxx::future<> linear_gather(const T* send_buf, size_t send_count, T* dest_buf, intrank_t root,
  ...
  if (tm.rank_me() == root) {
    ...
    rpc_ff(tm, root,
           [send_count, root, &tm](DO& dist_prom, intrank_t from_rrank, upcxx::view<T> payload) {
             assert(tm.id() == dist_prom.team().id());
           ...
           dist_prom, rrank_me, upcxx::make_view(send_buf, send_buf + send_count));
  ...
  for (intrank_t level = 0; level < num_levels; level++) {
    ...
    if (is_sending) {
      ...
      auto fut = fut_buf_filled.then([rrank, root, level, offset, send_count, have_received, &tm, &dist_workflows,
        ...
        rpc_ff(tm, dest_rank,
               [](DWs& dist_workflows, intrank_t level, upcxx::view<T> payload) {
                 ...
                 return fut_buf.then([&recv_workflow, payload, &dist_workflows, level](ShBuffer dest_buf) {
                   assert(dest_buf);
                   DBG_VERBOSE("receiving level=", level, " dest_buf->size()=", dest_buf->size(),
                               " capacity=", dest_buf->capacity(), " payload=", payload.size(),
                               " workflow.data=", (void*)recv_workflow.data, "\n");

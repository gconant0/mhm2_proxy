...
upcxx::future<> reduce_prefix_ring(const T *src, T *dst, size_t count, const BinaryOp &op, const upcxx::team &team = world(),
  ...
  if (return_final_to_first || next_rank != team.rank_n()) {
    ...
    ret_fut = ret_fut.then([next_rank, dst, count, sh_dist_data, &team]() -> upcxx::future<> {
      ...
      rpc_ff(team, next_rank % team.rank_n(),
    ...
    step = step.then(
        [sh_dist_data, src = src, dst = dst, count = count, sh_scratch = sh_scratch, my_node = my_node, &proms, &team, &op = op]() {
          ...
          if (my_node.parent < my_node.me) {
            ...
            send_to_parents = rpc(team, my_node.parent,
...
upcxx::future<> reduce_prefix_binary_tree_down(ShDistData<T, BinaryOp> sh_dist_data) {
  ...
  step = when_all(step, rpcs_sent, proms.get_down_future()).then([
    sh_dist_data, src = src, dst = dst, count = count, my_node = my_node, return_final_to_first = return_final_to_first, &proms,
    &team
  ]()->upcxx::future<> {
    assert(proms.up_ready());
...
upcxx::future<> reduce_prefix_binary_tree(const T *src, T *dst, size_t count, const BinaryOp &op, const upcxx::team &team = world(),
  ...
  for (auto i = 0; i < num_chunks; i++) {
    ...
    all_down =
        when_all(all_up, all_down).then([sh_dist_data]() -> upcxx::future<> { return reduce_prefix_binary_tree_down(sh_dist_data); });
  }
...
template <typename T, typename BinaryOp>
upcxx::future<> reduce_prefix_auto_choice_local(const T *src, T *dst, size_t count, const BinaryOp &op, const upcxx::team &team = world(),
                                         bool return_final_to_first = false, const upcxx::team &lteam = local_team()) {
  ...
  if (lteam.rank_me() == 0) {
    ...
    DBG_VERBOSE("Shifting skip node reductions by 1 node to shift_leader=", shift_leader, "\n");
    rpc_ff(team, shift_leader,
           [count, &lteam](NodePrefixDO &node_prefix, view<T> vals) -> upcxx::future<> {
             DBG_VERBOSE("Shifted node_prefix.\n");
             auto & [_sh_scratch, _prom_skip_node_reduce, _prom_node_prefix_shift] = *node_prefix;
             // needed for cray/clan
             auto &sh_scratch = _sh_scratch;
             auto &prom_skip_node_reduce = _prom_skip_node_reduce;
             auto &prom_node_prefix_shift = _prom_node_prefix_shift;
             assert(lteam.rank_me() == 0);
  ...
  upcxx::future<> fut_final_prefix = fut_shift.then([sh_scratch, src, dst, count, local_n, return_final_to_first, &team, &lteam, &op]() {
...
template <typename T, typename BinaryOp>
upcxx::future<T> reduce_prefix(const T elem, const BinaryOp &op, const upcxx::team &team = world(), bool return_final_to_first = false) {
...
#define MACRO_REDUCE_PREFIX(TYPE, OP, MODIFIER)                                                                                    \
  ...
  MODIFIER upcxx::future<> reduce_prefix_ring<TYPE, OP>(const TYPE *, TYPE *, size_t, const OP &, const upcxx::team &, bool, intrank_t);  \
  ...
  MODIFIER upcxx::future<> reduce_prefix_binary_tree<TYPE, OP>(const TYPE *, TYPE *, size_t, const OP &, const upcxx::team &, bool,       \
                                                        intrank_t, size_t);                                                        \
  MODIFIER upcxx::future<> reduce_prefix_auto_choice_local<TYPE, OP>(const TYPE *, TYPE *, size_t, const OP &, const upcxx::team &, bool, \
                                                              const upcxx::team &);                                                \
  MODIFIER upcxx::future<> reduce_prefix_auto_choice<TYPE, OP>(const TYPE *, TYPE *, size_t, const OP &, const upcxx::team &, bool, int); \

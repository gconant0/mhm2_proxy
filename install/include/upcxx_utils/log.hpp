...
#define INFO_OR_LOG_STREAM                           \
  (upcxx::rank_me() < upcxx::local_team().rank_n() ? \
       std::cout :                                   \
...
#define LOG_LINE_LABEL "[", upcxx::rank_me(), "] <", __FILENAME__, ":", __LINE__, "> "
...
#define LOG(...)                                                                                                    \
  do {                                                                                                              \
    upcxx_utils::logger(LOG_OR_DBG_STREAM, false, false, rank_me() % local_team().rank_n() == 0, LOG_LINE_TS_LABEL, \
                        ##__VA_ARGS__);                                                                             \
  } while (0)
...
#define INFO_OR_LOG(...)                                                                                             \
  do {                                                                                                               \
    upcxx_utils::logger(INFO_OR_LOG_STREAM, false, false, rank_me() % local_team().rank_n() == 0, LOG_LINE_TS_LABEL, \
                        ##__VA_ARGS__);                                                                              \
  } while (0)
...
std::vector<string> find_rank_files(string &fname_list, const string &ext, bool cached_io);
...
const upcxx::team &barrier_wrapper(const upcxx::team &team = upcxx::world());
